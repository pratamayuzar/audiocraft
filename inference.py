...
magnet = audiocraft.models.MAGNeT.get_pretrained(CHECKPOINT_DIR)  # No need for .cuda()
...
torchaudio.save(OUTPUT_PATH, torch.tensor(output[0]), sample_rate=32000)

print(f"✅ Audio file saved at: {OUTPUT_PATH}")

# Play audio (if running in a Jupyter notebook)
display_audio(torch.tensor(output[0]), sample_rate=32000)
